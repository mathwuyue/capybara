...
def delete_session(user_id, session_id):
    UserHistory.update(is_deleted=True).where(
        (UserHistory.user_id == user_id) & (UserHistory.session_id == session_id)
    ).execute()
...
def get_history(user_id, session_id, limit=40):
    ...
    if results:
        history = {
            ...
            "history": [
                HistoryItem(role=h.role, content=h.message).model_dump()
...
def update_history(
    ...
    user_meta: dict,
    state: str,
    records: HistoryItem | list[HistoryItem],
) -> History:
    """
