...
class Memory:
    ...
    def get_memory(cls, organization, limit=0):
        try:
            if limit:
                result = (
                    MemoryModel.select(MemoryModel.text, MemoryModel.ans)
                    .where(MemoryModel.organization == organization)
                    .limit(limit)
                )
            else:
                result = MemoryModel.select(MemoryModel.text, MemoryModel.ans).where(
                    MemoryModel.organization == organization
                )
    ...
    def search_memory(cls, organization, query, topk=10):
        ...
        mem_query = (
            MemoryModel.select(MemoryModel.id, MemoryModel.text, MemoryModel.ans)
            .where(MemoryModel.organization == organization)
